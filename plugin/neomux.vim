...
function! s:EnsureTmuxModule() abort
    if exists('*neomux#tmux#Socket')
        return
    endif

    let l:tmux_file = fnamemodify(s:this_folder, ':h') . '/autoload/neomux/tmux.vim'
    if filereadable(l:tmux_file)
        execute 'source ' . fnameescape(l:tmux_file)
    endif
endfunction
...
function! s:NeomuxMain()
    ...
    if g:neomux_enable_tmux
        call s:EnsureTmuxModule()
        call neomux#tmux#CleanDeadSocketsOnStartup()
    endif
...
function! NeomuxTerm(...)
    ...
    if g:neomux_enable_tmux && !exists("l:term_cmd")
        ...
        if !executable('tmux')
            ...
            call neomux#tmux#EnsureSessionVars()

            " Get next session number and create session name
            let l:session_num = neomux#tmux#GetNextSessionNum(g:neomux_tmux_socket_file)
            let l:tmux_session_name = 'nmux_' . l:session_num

            " Create the tmux command
            let l:term_cmd = neomux#tmux#CreateSession(g:neomux_tmux_socket_file, l:tmux_session_name)
    ...
    if l:is_tmux_term
        ...
        call neomux#tmux#SetSessionEnvironment(b:neomux_tmux_socket, b:neomux_tmux_session)

        " Generate default name and set neovim buffer name (handles uniqueness)
        let l:default_name = neomux#tmux#GenerateDefaultTerminalName()
        let l:name_result = neomux#tmux#SetNeomuxBufferName(l:bufnr, l:default_name)
        ...
        call neomux#tmux#SetWindowNameWithRetry(b:neomux_tmux_socket, b:neomux_tmux_session, l:name_result.final, 10)

        " Start autosave timer if not already running
        call neomux#tmux#StartAutosaveTimer()
    endif
endfunction
...
" ============================================================================
" Tmux Integration Compatibility Wrappers
" ============================================================================

function! s:CallTmux(name, args) abort
    call s:EnsureTmuxModule()
    return call('neomux#tmux#' . a:name, a:args)
endfunction

function! NeomuxTmuxSocket() abort
    return s:CallTmux('Socket', [])
endfunction

function! NeomuxTmuxSessionName() abort
    return s:CallTmux('SessionName', [])
endfunction

function! NeomuxSessionDisplayName() abort
    return s:CallTmux('SessionDisplayName', [])
endfunction

function! NeomuxTerminalName(...) abort
    return s:CallTmux('TerminalName', a:000)
endfunction

function! NeomuxIsTerminal(...) abort
    return s:CallTmux('IsTerminal', a:000)
endfunction

function! NeomuxTmuxListSessions() abort
    return s:CallTmux('ListSessions', [])
endfunction

function! NeomuxTmuxListSessionsForPicker() abort
    return s:CallTmux('ListSessionsForPicker', [])
endfunction

function! NeomuxTmuxReconnect(session_name) abort
    return s:CallTmux('Reconnect', [a:session_name])
endfunction

function! NeomuxTmuxReconnectPicker() abort
    return s:CallTmux('ReconnectPicker', [])
endfunction

function! NeomuxTmuxKillServer() abort
    return s:CallTmux('KillServer', [])
endfunction

function! NeomuxTmuxClean() abort
    return s:CallTmux('Clean', [])
endfunction

function! NeomuxRenameTerminal(name) abort
    return s:CallTmux('RenameTerminal', [a:name])
endfunction

function! NeomuxRenameTerminalPrompt() abort
    return s:CallTmux('RenameTerminalPrompt', [])
endfunction

function! NeomuxRenameSession(name) abort
    return s:CallTmux('RenameSession', [a:name])
endfunction

function! NeomuxRenameSessionPrompt() abort
    return s:CallTmux('RenameSessionPrompt', [])
endfunction

function! NeomuxSaveSession() abort
    return s:CallTmux('SaveSession', [])
endfunction

function! NeomuxRestoreSession(...) abort
    return s:CallTmux('RestoreSession', a:000)
endfunction

function! NeomuxAutosaveStatus() abort
    return s:CallTmux('AutosaveStatus', [])
endfunction

function! NeomuxBufferPicker() abort
    return s:CallTmux('BufferPicker', [])
endfunction

function! NeomuxGetConnectCommand(...) abort
    return s:CallTmux('GetConnectCommand', a:000)
endfunction

function! NeomuxCopyConnectCommand(...) abort
    return s:CallTmux('CopyConnectCommand', a:000)
endfunction
